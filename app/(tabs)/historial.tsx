...
export default function Historial() {
  ...
  const exportar = () => {
    const reporte = `📊 REPORTE DE ESTACIONAMIENTO\n\nFecha: ${new Date().toLocaleString("es-MX")}\n\nESTADÍSTICAS GENERALES\n${"─".repeat(30)}\nTotal de registros: ${registros.length}\nVehículos activos: ${activos}\nVehículos completados: ${registros.length - activos}\nIngresos totales: $${ingresos} MXN`;
    Alert.alert("📊 Reporte generado", reporte);
  };
  ...
  return (
    <View style={styles.container}>
      ...
      <View style={styles.statsRow}>
        <View style={styles.statCard}><Text style={styles.statNum}>{registros.length}</Text><Text style={styles.statLabel}>Total</Text></View>
        <View style={[styles.statCard, { backgroundColor: "#f0fdf4" }]}><Text style={[styles.statNum, { color: "#22C55E" }]}>{activos}</Text><Text style={styles.statLabel}>Activos</Text></View>
        <View style={[styles.statCard, { backgroundColor: "#fff7ed" }]}><Text style={[styles.statNum, { color: "#F59E0B" }]}>${ingresos}</Text><Text style={styles.statLabel}>Ingresos</Text></View>
      </View>

      {/* Filtros */}
      <View style={styles.filtrosCard}>
        <Text style={styles.filtroTitle}>🔽 Filtros</Text>
        <Text style={styles.filtroLabel}>Tipo de vehículo</Text>
        <View style={styles.filtroRow}>
          {(["todos", "auto", "moto"] as const).map((f) => (
            <TouchableOpacity key={f} style={[styles.filtroBtn, filtroTipo === f && styles.filtroBtnActive]} onPress={() => setFiltroTipo(f)}>
              <Text style={[styles.filtroText, filtroTipo === f && styles.filtroTextActive]}>
                {f === "todos" ? "Todos" : f === "auto" ? "Autos" : "Motos"}
              </Text>
            </TouchableOpacity>
          ))}
        </View>
      ...
      <FlatList
        ...
        renderItem={({ item }) => (
          <View style={[styles.itemCard, item.activo && styles.itemActivo]}>
            ...
            <View style={styles.itemBody}>
              <Text style={styles.itemInfo}>Entrada: <Text style={styles.itemVal}>{formatHora(item.entrada)}</Text></Text>
              {item.salida && <Text style={styles.itemInfo}>Salida: <Text style={styles.itemVal}>{formatHora(item.salida)}</Text></Text>}
              {item.lat ? <Text style={styles.itemInfo}>Ubicación: <Text style={styles.itemVal}>{item.lat?.toFixed(4)}, {item.lng?.toFixed(4)}</Text></Text> : null}
        ...
        ListFooterComponent={
          <TouchableOpacity style={styles.exportBtn} onPress={exportar}>
            <Text style={styles.exportText}>⬇️ Exportar Reporte</Text>
          </TouchableOpacity>
        }
      />
    </View>
...
const styles = StyleSheet.create({
  ...
  filtroLabel: { fontSize: 12, color: "#666", marginBottom: 8 },
